...
namespace r2
{
	...
	void Direction8SequentialState::ConvertState2Point( const eState state, int* out_x, int* out_y )
	{
		switch( state )
		{
		case eState::Up:
	...
	void Direction8SequentialState::ConvertPoint2State( const int x, const int y, eState& out_state )
	{
		if( x == 0 && y >= 1 )
		{
			out_state = eState::Up;
	...
	bool Direction8SequentialState::IsDiagonalDirection8State( const eState state )
	{
		return (
