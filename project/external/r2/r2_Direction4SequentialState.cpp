...
namespace r2
{
	...
	void Direction4SequentialState::ConvertState2Point( const eState state, int* out_x, int* out_y )
	{
		switch( state )
		{
		case eState::Up:
	...
	Direction4SequentialState::eState Direction4SequentialState::ConvertPoint2State( const int x, const int y )
	{
		if( 1 <= x )
		{
			return eState::Right;
